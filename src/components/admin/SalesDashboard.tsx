...
import { Download, FileText, KeyRound, Layers3, Loader2, LogOut, RefreshCw, Search } from "lucide-react";
...
export default function SalesDashboard() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-xs font-semibold uppercase tracking-wider text-primary-600">Private sales workspace</p><h1 className="mt-1 text-3xl font-bold text-secondary-900">Customer inquiries</h1></div><div className="flex flex-wrap gap-2"><Link href="/admin/catalog" className="flex items-center gap-2 rounded-lg border border-secondary-200 bg-white px-3 py-2 text-xs font-semibold text-secondary-600"><Layers3 className="h-4 w-4" />Catalog</Link><Link href="/admin/quotes" className="flex items-center gap-2 rounded-lg border border-secondary-200 bg-white px-3 py-2 text-xs font-semibold text-secondary-600"><FileText className="h-4 w-4" />Quote history</Link><button onClick={exportCsv} className="flex items-center gap-2 rounded-lg border border-secondary-200 bg-white px-3 py-2 text-xs font-semibold text-secondary-600"><Download className="h-4 w-4" />Export CSV</button><button onClick={signOut} className="flex items-center gap-2 rounded-lg border border-secondary-200 bg-white px-3 py-2 text-xs font-semibold text-secondary-600"><LogOut className="h-4 w-4" />Sign out</button></div></div>
